...
public:
	Cluster(int id_cluster, Point point)
	{
		...
		for(int i = 0; i < total_values; i++)
			central_values.push_back(point.values[i]);
...
public:
	...
	void run(vector<Point>& points)
	{
				...
				for(int j = 0; j < total_values; j++)
				{
					...
					if(total_points_cluster > 0)
					{
						for(int p = 0; p < total_points_cluster; p++)
							sum += clusters[i].getPoint(p).values[j];
		...
		for(int i = 0; i < K; i++)
		{
			...
			{
				// cout << "Point " << clusters[i].getPoint(j).id_point + 1 << ": ";
				// for(int p = 0; p < total_values; p++)
				// 	cout << clusters[i].getPoint(j).values[p] << " ";

				// string point_name = clusters[i].getPoint(j).name;
